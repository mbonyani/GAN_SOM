...
from AE import cnn_autoencoder, mlp_autoencoder
from metrics import quantization_error, cluster_acc, cluster_purity, topographic_error
...
class DESOM:
    ...
    def __init__(self, encoder_dims, map_size, AE_type='cnn'):
    ...
    def initialize(self, ae_act='relu', ae_init='glorot_uniform'):
        ...
        # Create AE models
        if self.AE_type=='cnn':
            self.autoencoder, self.encoder, self.decoder = cnn_autoencoder(self.input_dim, 32)
        else:
        ...
        for ite in range(iterations):
            ...
            if ite % eval_interval == 0:
                ...
                if y_train is not None:
                    ...
                    logdict['nmi'] = normalized_mutual_info_score(y_batch, y_pred)
                    logdict['ari'] = adjusted_rand_score(y_batch, y_pred)
                ...
                if y_val is not None:
                    ...
                    logdict['nmi_val'] = normalized_mutual_info_score(y_val_batch, y_val_pred)
                    logdict['ari_val'] = adjusted_rand_score(y_val_batch, y_val_pred)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', default='fmnist', choices=['mnist', 'fmnist', 'usps', 'reuters10k'])
    ...
    parser.add_argument('--map_size', nargs='+', default=[4, 4], type=int)
